...
{
	const std::vector<const DirectoryEntry>& dirs = rootDir.getDirectories();

	WORD wDataRegionOffset = rootDir.getFiles().size() * RSFS_FILEENTRY_SIZE + dirs.size() * RSFS_DIRECTORYENTRY_SIZE;

	for(std::vector<const DirectoryEntry>::const_iterator it = dirs.begin(); it != dirs.end(); it++)
...
void RsfsFormatter::writeDirectory(const DirectoryEntry& dir, const WORD wDataRegionOffset, std::ostream& fileIndexBuffer, std::ostream& fileRawDataBuffer)
{
	...
	const std::vector<const DirectoryEntry>& dirs = dir.getDirectories();
	const std::vector<const FileEntry>& files = dir.getFiles();
	...
	for(std::vector<const FileEntry>::const_iterator it = files.begin(); it != files.end(); it++)
	{
...
void RsfsFormatter::create(std::ostream& out, const DirectoryEntry& rootDir)
{
	...
	out.write(reinterpret_cast<const char*>(&numRootEntries), sizeof(WORD));
