...
protected:
	...
	static void writeHeader(std::ostream& out, const std::vector<Bitmap*>& allocationMaps, const WORD wRootFlat);
	
	void addDirectory(std::ostream& out, const DirectoryEntry& rootDir, const unsigned int iReservedEntries, 
						LafsFlat& lastFlat, std::vector<Bitmap*>& allocationMaps, const bool isInlineDirectory = false);
